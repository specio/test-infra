...
Options:
  ...
  -w, --wait-timer <time>           Wait time in queue [default: 20]
...
class Trigger:
    ...
    def waiting_for_job_to_start(self, queue_url):
        ...
        while waiting_for_job:
            if timer == 0:
                print("** timed out waiting for job to start")
                exit(1)

            print('Checking Queue at: ' + queue_url + '; Checks remaining: ' + str(timer))
            queue_request = requests.get(queue_url)
            if queue_request.json().get("why"):
                # Decrease timer count and message (take longer and longer to limit messages in log)
                timer -= 1
                message = 'Sleeping for %ss due to build having why: %s' % \
                          (self.sleep * (self.timer - timer), queue_request.json().get("why").encode('utf-8').strip())
                print(message)
                sleep(self.sleep * (self.timer - timer))
                continue
    ...
    def console_output(self, job_url):
        ...
        while stream_open:
            ...
            if check_job_status > 1:
                ...
                # print('Received HTTP ' + str(job_requests.status_code))
                # print(job_requests.json())
    ...
    def main(self):
        ...
        # job_url = 'https://oe-ci.sgxoeprd.adsdcsp.com/job/Pipelines/job/Agnostic-SGX-FLC-Linux/20/'
