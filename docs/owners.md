...
description: |
  ...
  Open Enclave codebase and serve as the implementation mechanism for the two-phase
  code review process used by the project.
---

## Overview

OWNERS files are used to designate responsibility over different parts of the Open Enclave codebase.
...
process. Our OWNERS files were inspired by the Kubernetes project.
...
- Technically, anyone who is a member of the Open Enclave GitHub organization can drive-by `/lgtm` a
  PR
...
Open Enclave uses the Prow Blunderbuss plugin and Tide.
