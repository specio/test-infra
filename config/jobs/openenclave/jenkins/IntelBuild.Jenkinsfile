                ...
                script{
                    ...
                    stage("Ubuntu ${LINUX_VERSION} Build - ${BUILD_TYPE}"){
                        try{
                            ...
                            runner.ContainerBuild("oetools-full-18.04:${DOCKER_TAG}","${BUILD_TYPE}","${COMPILER}","--device /dev/sgx --device /dev/mei0 --cap-add=SYS_PTRACE --user=root --volume /jenkinsdata/workspace/Pipelines/OpenEnclave-TestInfra/build/openenclave:/jenkinsdata/workspace/Pipelines/OpenEnclave-TestInfra/build/openenclave --env https_proxy=http://proxy-mu.intel.com:912 --env http_proxy=http://proxy-mu.intel.com:911 --env no_proxy=intel.com,.intel.com,localhost","${EXTRA_CMAKE_ARGS}","${PULL_NUMBER}")
                        } catch (Exception e) {
